...
<div class="definition">Regret is defined as
...
RFTL defines $\mathbf x_{t}=\argmin_{\mathbf{x}} F_{t-1}(\mathbf x)$ so $F_{t-1}(\mathbf x_t)$ is the optimum and first over characterization[^foc] becomes
...
[^BTL]: Note that we additionally use $f_0(\mathbf x^*) \geq f_o(\mathbf x_1)$ since BTL only applies to $t=1,...,T$.
...
<!-- Algo -->
$$
\begin{array}{l}{\mathbf x_{t+1}=\argmin_{\mathbf x}\left\langle \mathbf u_{t}, \mathbf x\right\rangle+\frac{1}{\eta} \psi(\mathbf x)} \\ {\mathbf u_{t+1}=\mathbf u_{t}+\mathbf g_{t}}\end{array}
$$
...
<!-- TODO: styling, reference stability, algo style, OGD, EG  -->

<!-- OMD, mirror map interpretation of FTRL -->
<!-- clear up terminlogy -->
